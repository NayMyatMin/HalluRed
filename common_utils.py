...
def get_full_model_name(model_name: str):
    ...
    if "vicuna7b" in model_name:
        ...
        raise ValueError(
            "Unknown model identifier. Use one of: vicuna7b, vicuna13b, llama, llama-3, pythia, guanaco, mistral, falcon"
        )
    return name
...
def load_model_and_tokenizer(
    ...
    if tokenizer.pad_token is None:
        # Prefer EOS as PAD to avoid resizing embeddings or introducing OOV tokens
...
def get_attn_eig_prod(attns, layer_num=15, tok_lens=[], use_toklens=True):
    ...
    for i in range(len(attns)):  # iterating over number of samples
        ...
        for attn_head_num in range(len(attns[i][layer_num])):  # iterating over number of attn heads
            ...
            diag = torch.clamp(torch.diagonal(Sigma, 0), min=1e-12)
            eigscore += torch.log(diag).mean()
