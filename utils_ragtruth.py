...
def get_ragtruth_data(n_samples=200):
    train_data = []
    test_data = []
    rag_path = "rag_truth/dataset/response_summary.jsonl"
    if not os.path.exists(rag_path):
        raise FileNotFoundError(
            "rag_truth/dataset/response_summary.jsonl not found. Download the RAGTruth dataset and place the JSONL at rag_truth/dataset/response_summary.jsonl"
        )
    with jsonlines.open(rag_path) as f:
        for row in f:
