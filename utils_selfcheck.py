...
# Prefer HF_DATASETS_CACHE if provided; fallback to project-local cache
_cache_dir = os.getenv("HF_DATASETS_CACHE")
if _cache_dir:
    datasets.config.DOWNLOADED_DATASETS_PATH = Path(_cache_dir)
else:
...
def get_selfcheck_data(n_samples=1000):
    ...
    try:
        ...
        # If select is unavailable for any reason, fallback to slicing materialization
        eval_ds = eval_ds[:n_samples]
    return eval_ds, []
...
def get_scores_dict(model_name_or_path, data, mt_list, args):
    ...
    # Original behavior: try to load wiki_bio test split for concept context; fallback if it fails
    wikibio = None
    try:
        wikibio = datasets.load_dataset("wiki_bio", split="test")
    except Exception as e:
        print("[WARN] Failed to load wiki_bio test split; falling back to deriving concept from wiki_bio_text. Error:", e)
    ...
    for i in tqdm(range(len(data))):
        ...
        if wikibio is not None:
            concept = wikibio[wiki_id]["input_text"]["context"].strip()
        else:
            # Fallback: derive a short "concept" from available text
            wb_text = row.get("wiki_bio_text") or ""
            if isinstance(wb_text, str) and len(wb_text) > 0:
                concept = wb_text.split("\n")[0].split(",")[0].split("(")[0].strip()
                if len(concept) == 0:
                    concept = "the subject"
            else:
                concept = "the subject"
